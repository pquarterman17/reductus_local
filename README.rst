...
Method 2: Offline install (no internet)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The ``vendor/`` directory contains pre-downloaded wheel files for Windows x64
and Linux x64 (Python 3.13). To install on a machine with no internet access,
copy the repo (including ``vendor/``) to the target machine and run::

    pip install --no-index --find-links vendor/ ".[all]"

To refresh or add wheels for a different platform or Python version, run
``pip download`` on an internet-connected machine::

    pip download -d vendor/ --platform <PLATFORM> --python-version 3.13 --only-binary=:all: ".[all]" setuptools wheel

where ``<PLATFORM>`` is e.g. ``win_amd64``, ``manylinux2014_x86_64``, or
``macosx_11_0_arm64``.
...
Method 4: Run from repo (development)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Method 5: Run from repo (production)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
